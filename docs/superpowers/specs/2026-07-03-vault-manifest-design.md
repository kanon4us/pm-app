...
- **Degraded-manifest guard vs. mass deletions (ops note):** the cron skips the manifest write when the new manifest is empty or shrinks total `file_count` by more than half vs. the existing one (protects `main` from a partially-failed GitHub snapshot). A *legitimate* deletion of >50% of the vault's docs therefore suppresses refreshes — the cron logs `[vault-cron] manifest looks degraded — skipping write` weekly and the manifest goes stale (soft consequence: assess falls back to live search). Recovery: manually delete or hand-trim `MANIFEST.json` on `main`, then let the next run rewrite it.
