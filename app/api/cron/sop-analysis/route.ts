...
- CRITICAL when the section is intake_prompt: the bot uses intake_prompt VERBATIM as its ENTIRE system prompt and requires the model to return JSON containing updated_schema, bot_response, and "Respond with valid JSON". So "new" MUST be the COMPLETE revised prompt — take current_intake_prompt and apply your change to it, returning the whole thing. NEVER return only the changed sentence, a fragment, or a summary: a partial intake_prompt drops the JSON contract, breaks the live bot, and is auto-rejected on approval. The result must be at least as long as current_intake_prompt and still contain the tokens updated_schema, bot_response, and "Respond with valid JSON". Prefer expressing an additive rule as an extra instruction appended to the existing prompt rather than rewriting it.
